...
const Header: React.FC<HeaderProps> = ({ onNavigateHome, onNavigate }) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 w-full bg-brand-light dark:bg-brand-dark selection:bg-orange-500 selection:text-white">
        {/* Desktop Header */}
        <nav className="hidden md:flex items-center justify-between px-6 py-4 md:py-6 max-w-7xl mx-auto">
          <button 
          ...
          <div className="flex items-center gap-4 md:gap-8">
            <div className="hidden lg:flex items-center gap-8 text-[10px] font-black uppercase tracking-widest text-zinc-400">
              <a href="#" onClick={() => handleNavigate('guide')} className="hover:text-zinc-900 dark:hover:text-white transition-colors">Guide</a>
              <a href="#" onClick={() => handleNavigate('packages')} className="hover:text-zinc-900 dark:hover:text-white transition-colors">Packages</a>
              <a href="#" onClick={() => handleNavigate('showcase')} className="hover:text-zinc-900 dark:hover:text-white transition-colors">Showcase</a>
            </div>
          </div>
        </nav>

        {/* Mobile Header */}
        <nav className="md:hidden flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
          <button 
            onClick={onNavigateHome}
            className="flex items-center gap-1 md:gap-2 cursor-pointer hover:opacity-80 transition-opacity"
          >
            <div className="w-8 h-8 rounded-lg shadow-xl overflow-hidden">
              <img src="/icon.png" alt="Cool Buttons" className="w-full h-full object-cover" />
            </div>
            <h1 className="text-lg font-black tracking-tighter dark:text-white">
              COOL<span className="text-orange-500">BUTTONS</span>
            </h1>
          </button>
